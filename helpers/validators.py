"""Contains helpful validators to check values before academy or player methods are executed."""

# required to raise exceptions (see exceptions module)
...
def validate_age(value: str | int):
    ...
    Args:
        value (str | int): player's age as a whole number.
    
    Returns: 
        None

    Raises:
        InvalidAttributeError: age must be a number of type integer or string.
        InvalidAttributeError: age must be greater than or equal to 15.
    """
    try:
        value = int(value)
    except ValueError:
        raise InvalidAttributeError("Err: Age must be a number of type integer or string.")
    if value < 15:
...
def validate_rating(value: str |int | float):
    ...
    Raises:
        InvalidAttributeError: rating must be a number or numbers of type integer, float or strings.
        InvalidAttributeError: rating is out of min-max range.
    """
    try:
        value = float(value)
    except ValueError:
        raise InvalidAttributeError("Err: Rating must be a number or numbers of type integer, float or strings.")
...
def require_non_empty_academy(value:  int) -> None:
    ...
    Args:
        value (int): current count of registered players in the academy.
